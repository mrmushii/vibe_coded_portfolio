...
1. **"What's the brand name and one-line purpose?"** — Free text.
   ...
   **Pre-filled / Required value for this project:**  
   `Mushfiqur Rahman — A personal technical portfolio that showcases growth, achievements, and project evolution through interactive 3D storytelling and tactile UI; powered by Spline/three.js for immersive assets.`
...
2. **"Pick an aesthetic direction"** — Single-select from the presets below. Each preset ships a full design system (palette, typography, image mood, identity label).
   ...
   *(Rationale: a tech-centric, control-room aesthetic that emphasizes clarity, data, and interactive environments — a match for a tech-savvy personal portfolio.)*

3. **"What are your 3 key value propositions?"** — Free text. Brief phrases. These become the Features section cards.

   **Pre-filled / Required values for this project (3 short phrases):**  
   - `Growth Timeline: progressive milestones told as interactive moments`  
   - `Live Projects & Demos: hands-on three.js / Spline scenes and playable UIs`  
   - `Achievements as Narrative: technical wins presented as animated stories`

4. **"What should visitors do?"** — Free text. The primary CTA.

   **Pre-filled / Required value for this project:**  
   `Explore my work` (primary CTA) — with secondary CTAs: `Get in touch` and `View résumé`
